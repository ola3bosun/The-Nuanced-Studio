...
function parseHSL(hslStr: string): { h: number; s: number; l: number } {
  ...
  if (!match) return { h: 67, s: 100, l: 50 }; // Default to Chartreuse
...
export default function GlowButton({
  ...
  className = '',
  edgeSensitivity = 30,
  glowColor = '67 100 50', // TNS Chartreuse HSL
  backgroundColor = '#010101', // TNS Black
  borderRadius = 2, // Sharp/slight curve
  glowRadius = 10,
  glowIntensity = 1.0,
  coneSpread = 15,
  animated = false,
  ...
  return (
    <button
      ...
      className={`relative grid isolate border border-[#FFFCF5]/15 cursor-pointer px-6 py-2.5 transition-colors duration-300 ${className}`}
      style={{
    ...
    >
      ...
      {/* Button Text */}
      <span className="relative z-[2] text-[14px] font-medium tracking-wide text-[#FFFCF5] whitespace-nowrap">
        {text}
      </span>
    </button>
  );
